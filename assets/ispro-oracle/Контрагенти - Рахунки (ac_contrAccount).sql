...
with contractor AS (
	select /*+ MATERIALIZE */
		distinct u1.kpuudr_cdplc "ID", u1.kpuudr_cdbank account
...
select
	...
	,PtnSch_Bcd "bankID"
	,case when length(PtnSch_Nsc) <> 0 then PtnSch_Nsc else '*0000000000000' end "code"
	,case when length(PtnSch_Kom) = 0 then 'Розрахунковий' else PtnSch_Kom end || ' ' || PtnSch_Nsc || ' UAH (' || b.bank_nm || ')' "description"
