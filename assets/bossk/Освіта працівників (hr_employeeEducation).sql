...
select 
	...
	case when date_finish is not null and date_finish > '1900-12-31' then cast(date_finish as date) else '' end dateIssue,
	-- forme + 1 educationForm,
	'' docSeries,
	"Certificate" docNumber,  
	left(coalesce(VPR_ED_DEPTS.Firm_Text, ''), 200) docIssuer,
	case when Certific_Date is not null and Certific_Date > '1900-12-31' then cast(Certific_Date as date) else '' end dateIssue,
