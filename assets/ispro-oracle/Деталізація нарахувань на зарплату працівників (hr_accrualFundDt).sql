...
WITH
    currentPeriod AS (
        SELECT
            ...
            END AS dateFrom
        FROM
            /*FIRM_SCHEMA*/ISPRO_8_PROD.vwCrtParm v1
            /*SYSSTE_BEGIN*/
            JOIN (
...
select
    ...
    '' "departmentID",
    case
        when KpuF_CdSch is not null
        and KpuF_CdSch > 0 then TO_CHAR (KpuF_CdSch)
        else ''
    end "accountID"
from
    /*FIRM_SCHEMA*/ISPRO_8_PROD.kpufa1
...
where
    kpuf_cdfnd <> 0
    and kpuf_datup BETWEEN ADD_MONTHS(TRUNC (ADD_MONTHS(SYSDATE, -12), 'YEAR'), -3) AND cp.dateFrom  - 1
group by
    kpuf_datup,
    kpuf_datrp,
    kpuf_tn,
