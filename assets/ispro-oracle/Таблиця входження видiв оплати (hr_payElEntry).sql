...
select ID ID, 
	payElID "payElID", 
	payElBaseID "payElBaseID", 
	TO_CHAR(dateFrom, 'YYYY-MM-DD') "dateFrom", 
	TO_CHAR(dateTo, 'YYYY-MM-DD') "dateTo", 
	entryType
from
(	
	select 
		min(t1.bookmark) ID
		,t1.paytv_cd payElID
		,t1.paytv_cdv payElBaseID
		,TO_CHAR(t1.paytv_dat, 'YYYY-MM-DD') dateFrom
		,case when t2.paytv_dat is null then '9999-12-31' else TO_CHAR(t2.paytv_dat - 1) end dateTo	
	...
	left join /*FIRM_SCHEMA*/ISPRO_8_PROD.payTvDat t2 on t2.paytv_part = t1.paytv_part 
		...
		and t2.paytv_dat = (
			...
			from /*FIRM_SCHEMA*/ISPRO_8_PROD.payTvDat t3
	...
	where t1.paytv_part = 1
		...
		and t1.paytv_cd in (
			select vo_cd 
			from /*FIRM_SCHEMA*/ISPRO_8_PROD.payVoList
		)
		and t1.paytv_cdv in (
			select vo_cd 
			from /*FIRM_SCHEMA*/ISPRO_8_PROD.payVoList
...
where t2.dateTo >= ADD_MONTHS(TRUNC(ADD_MONTHS(SYSDATE, -12), 'YEAR'), -3)
