...
async function getTableStruct(dbType, connection, tableName) {
    switch (dbType) {
        case DBtype.POSTGRES:
            return new Promise((resolve, reject) => {
                const queryText = `select column_name, data_type, character_maximum_length from INFORMATION_SCHEMA.COLUMNS where table_name ='${tableName.toLowerCase()}';`
                connection
                    .query(queryText)
                    .then((res) => {
                        res.rows.length
                            ? resolve(res.rows)
                            : reject(new Error('Table does not exists.'))
                    })
                    .catch((err) => reject(err))
            })
        case DBtype.MSSQL:
            return new Promise((resolve, reject) => {
                const queryText = `select column_name, data_type, character_maximum_length from INFORMATION_SCHEMA.COLUMNS where table_name ='${tableName}';`
                const tableStruct = []
