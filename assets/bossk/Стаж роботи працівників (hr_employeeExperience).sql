...
select 
	...
	max(dateTo) dateTo,
	cast(cast(max(dateTo) as datetime) - sum(DATEDIFF(day, dateFrom, dateTo)) as date) calcDate,
...
from (
	SELECT 
		...
		cast( case when st1.date_b between '1901-01-01' and GETDATE() then st1.date_b 
				when people.in_date between '1901-01-01' and GETDATE() then people.in_date
				else GETDATE() end
			as date) dateFrom,
		cast( case when st1.date_e between '1901-01-01' and GETDATE() then st1.date_e 
