...
-- select * from base.info_dep_history;
...
select base.uuid_bigint(d1.id::text) "ID",
    ...
    cast(h1.from_date as date) "dateFrom",
    case when h1.close_date is not null then cast(cast(h1.close_date as date) as varchar) else '' end "dateTo"
    --
    -- ,t1.name "typeName",
    -- NULLIF(regexp_replace(s1.short_name, '\D','','g'), '') "parentCode",
    -- s1.short_name "parentName",
    -- s1.long_name "parentFullName",
    -- t2.name "parentTypeName",
    -- t3.name "parentTypeName"
